...
LEAF_SUMMARY_PROMPT_PATH = "project_management/prompts/cact-leaf-summary.md"
DIR_SUMMARY_PROMPT_PATH = "project_management/prompts/cact-dir-summary.md"
MAX_LEAF_LINES = 200
        ...
        "children": [
          ...
          },
          {
            ...
            "children": [
              ...
              {"key": "project_management/prompts/cact-leaf-summary.md",
               "node_type": "source",
               "file_path": "project_management/prompts/cact-leaf-summary.md"},
              {"key": "project_management/prompts/cact-dir-summary.md",
               "node_type": "source",
               "file_path": "project_management/prompts/cact-dir-summary.md"},
            ],
          },
          {
...
def build_leaf_prompt(instruction, children_content):
...
def build_directory_prompt(instruction, children_summaries):
...
def load_prompt(path):
  """Read a prompt file from disk and return its text, stripped of whitespace.
...
def generate_summary(key, node, nodes, repo_root):
  ...
  if has_dir_children:
    ...
    instruction = load_prompt(repo_root / DIR_SUMMARY_PROMPT_PATH)
    child_summaries = []
    for ck in children:
      child = nodes[ck]
    ...
    prompt = build_directory_prompt(instruction, child_summaries)
  else:
    # All children are leaves
    instruction = load_prompt(repo_root / LEAF_SUMMARY_PROMPT_PATH)
    children_content = []
    for ck in children:
      child = nodes[ck]
      content = read_file_head(repo_root / child["file_path"])
      children_content.append((ck, content))
    prompt = build_leaf_prompt(instruction, children_content)
