...
1. Runs `python scripts/task_counter.py read` to get the current Task Counter. If ≥ 10, generates the architecture check prompt from architecture-check.md and stops; no task prompt is produced.
...
5. After confirmation: runs `python scripts/task_counter.py increment` to increment the Task Counter, updates manifest.md, runs `check_cdoc_coverage.py`, updates affected cdocs, reminds user to commit.
...
6. On PASS or PASS WITH NOTES: overwrites `artifacts/architecture-baseline.md` and all four `.mermaid` files; runs `python scripts/task_counter.py reset` to reset the Task Counter to 0.
